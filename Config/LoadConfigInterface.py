...
def salvar_configuracoes_json():
        editor_content = InterfaceConfigInterface.text_area_editor.get("1.0", tk.END)
        try:
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            if 'MostrarUsuario' in df.columns:
                ...
                raise KeyError("Chave 'MostrarUsuario' não encontrada no arquivo JSON")
            
            global SegundoPlano
            if 'SegundoPlano' in df.columns:
                SegundoPlano = df['SegundoPlano'].iloc[0]
            else:
                raise KeyError("Chave 'SegundoPlano' não encontrada no arquivo JSON")

    except (FileNotFoundError, ValueError, KeyError, IndexError) as e:
        Util.LogError("LoadConfigInterface",f"Erro ao ler ou processar o arquivo de configuração: {e}")
