...
def resetar_config():
    try:
        ...
        print(f"Erro: Você não tem permissão para apagar o arquivo '{file_path}'.")
...
def load_config():
    try:
        with open(file_path, "r") as f:
            ...
            sorted_config = OrderedDict(sorted(config_data.items()))  # Ordena as chaves em ordem alfabética
...
def iniciarConfig():
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            required_keys = ["TeclasDeAtalho","IniciarLimpeza","BuscarImagens","Fechar","Mostrar","Esconder"]
            for key in required_keys:
                if key not in data:
                    raise KeyError(f"Chave '{key}' não encontrada no arquivo JSON")
        ...
        Util.LogError("LoadConfigAtalhos",f"Erro ao ler ou processar o arquivo de configuração: {e}")
