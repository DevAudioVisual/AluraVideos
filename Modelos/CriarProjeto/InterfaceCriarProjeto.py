...
def interfaceCriarProjeto(tabview):
    ...
    CustomWidgets.CustomButton(framebuscar, text="Buscar", dica="Clique para buscar um diretório para o projeto.",Image=imagem_folder, command=changeDirectory).pack(pady=5,padx=5, fill="x", side="left")

    CustomWidgets.CustomLabel(dialog, text="Arquivo de vídeos (.rar ou .zip)", font=Styles.fonte_titulo).pack(pady=5, fill="x")
    ...
    def changeDirectoryVideos():
        ...
        try:
            ...
            if not os.path.isfile(filepath):  # Verifica se é um arquivo (e não um diretório)
                Util.LogError("InterfaceCriarProjeto",f"Arquivo não encontrado: {filepath}")
                return 

            extensoes_validas = [".rar", ".zip"]
            if os.path.splitext(filepath)[1].lower() not in extensoes_validas:
                Util.LogError("InterfaceCriarProjeto",f"O arquivo selecionado não é um RAR ou ZIP: {filepath}")
                return 
            ...
            Util.LogError("InterfaceCriarProjeto",f"Ocorreu um erro inesperado: {e}")
    
    framebuscarVideos = CustomWidgets.CustomFrame(dialog)
    framebuscarVideos.pack()    
    CustomWidgets.CustomEntry(framebuscarVideos, textvariable=ArquivoVideos, width=300).pack(pady=5, fill="x", side="left")
    imagem_folder = CustomWidgets.CustomImage("folder.png", 20, 20)
    def dialogo():
        global filepath
        dialogo = ctk.CTkInputDialog(text="Digite a URL completa", title="URL ")
    ...
    CustomWidgets.CustomButton(framebuscarVideos, text="Link Dropbox",dica=Util.quebrar_linhas("Clique para inserir o link publico para o dropbox"),Image=CustomWidgets.CustomImage("link.ico",20,20),command=dialogo).pack(pady=5,padx=5, fill="x", side="right")
    CustomWidgets.CustomButton(framebuscarVideos, text="Buscar",dica="Clique para buscar o arquivo de vídeos", Image=imagem_folder, command=changeDirectoryVideos).pack(pady=5,padx=5, fill="x", side="right")
    ...
    frameVerificarEextrairAudio = CustomWidgets.CustomFrame(frameBoxes)
    frameVerificarEextrairAudio.pack(side="left",fill="x")

    abrir_premiere = tk.BooleanVar(value=checks_vars["Abrir_Premiere"])
    CustomWidgets.CustomCheckBox(master=frameAbrirProjetoEPasta, text="Abrir Premiere",dica="Selecione para abrir o premiere ao criar.", variable=abrir_premiere).pack(pady=(10, 0),padx=5, anchor="w",fill="x")

    abrir_pasta = tk.BooleanVar(value=checks_vars["Abrir_pasta_do_projeto"])
    CustomWidgets.CustomCheckBox(master=frameVerificarEextrairAudio, text="Abrir pasta do projeto", dica="Selecione para abrir a pasta do projeto ao criar.", variable=abrir_pasta).pack(pady=(10, 0), anchor="w",fill="x")
    
    
    def mexeNao(): 
        verificar_videos.set(True)
        messagebox.showinfo("Aviso","Em breve")
    verificar_videos = tk.BooleanVar(value=checks_vars["Verificar_videos"])
    extrair_audio = tk.BooleanVar(value=checks_vars["Extrair_audio"])
    CustomWidgets.CustomCheckBox(master=frameAbrirProjetoEPasta, text="Extrair audio", dica=None,variable=extrair_audio).pack(pady=(10, 0),padx=5, anchor="w")
    CustomWidgets.CustomCheckBox(master=frameVerificarEextrairAudio, text="Verificar vídeos", dica="Em breve.",command=mexeNao,variable=verificar_videos).pack(pady=(10, 0), anchor="w")

    CustomWidgets.CustomLabel(dialog, text="Sub-pastas para criar:",dica="Selecione quais sub-pastas devem ser criadas.", font=Styles.fonte_titulo).pack(pady=10, fill="x")
    ...
    CustomWidgets.CustomButton(master=dialog, text="Criar", dica="Clique aqui para criar seu projeto.", command=CriarProjeto.criar_pastas,width=200).pack(anchor="center",pady=10,side=("left"),padx=10,fill="x")
    CustomWidgets.CustomCheckBox(master=dialog, text="Fechar ao criar",dica="Selecione para fechar o Software ao criar.", variable=fechar_ao_criar).pack(anchor="center",side=("left"),fill="x")
