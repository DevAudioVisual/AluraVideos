...
def ConfigCriarProjetoInterface(tabview):
    ...
    CustomWidgets.CustomLabel(
        framePrincipal, text="Predefinição para criar projetos", font=Styles.fonte_titulo).pack()

    frameDiretorios = ctk.CTkFrame(tabview.tab(
        "Config Criar projeto"), bg_color=Styles.cor_fundo, fg_color=Styles.cor_fundo)
    frameDiretorios.pack(side="top", fill="both", pady=10)

    frame = ctk.CTkFrame(tabview.tab("Config Criar projeto"),
                         bg_color=Styles.cor_fundo, fg_color=Styles.cor_fundo)
    frame.pack(side="left", fill="both", padx=10)

    frame2 = ctk.CTkFrame(tabview.tab("Config Criar projeto"),
                          bg_color=Styles.cor_fundo, fg_color=Styles.cor_fundo)
    frame2.pack(side="left", fill="both", padx=10)

    frame3 = ctk.CTkFrame(tabview.tab("Config Criar projeto"),
                          bg_color=Styles.cor_fundo, fg_color=Styles.cor_fundo)
    ...
    CustomWidgets.CustomLabel(
        frameDiretorios, text="Diretório padrão do projeto").pack(side="left")

    CustomWidgets.CustomEntry(
        frameDiretorios, textvariable=diretorio_padrao, width=300).pack(side="left", padx=10)
    ...
    fechar_ao_criar = tk.BooleanVar(
        value=config_data.get("fechar_ao_criar", True))

    checks_vars = {key: tk.BooleanVar(
        value=value) for key, value in config_data.get("checks", [{}])[0].items()}
    sub_pastas_vars = {key: tk.BooleanVar(
        value=value) for key, value in config_data.get("subpastas", [{}])[0].items()}
    ...
    CustomWidgets.CustomLabel(
        frame, text="Processos", font=Styles.fonte_titulo).pack(fill="both", pady=10)
    for key, var in checks_vars.items():
        CustomWidgets.CustomCheckBox(
            frame, key, var, update_config_from_widgets).pack(fill="both", pady=10)

    CustomWidgets.CustomLabel(
        frame2, text="Sub-pastas", font=Styles.fonte_titulo).pack(fill="both", pady=10)
    for key, var in sub_pastas_vars.items():
        CustomWidgets.CustomCheckBox(
            frame2, key, var, update_config_from_widgets).pack(fill="both", pady=10)

    CustomWidgets.CustomLabel(frame3, text="Fechar ao criar",
                              font=Styles.fonte_titulo).pack(fill="both", pady=10)
    fechar_ao_criar_checkbox = CustomWidgets.CustomCheckBox(
        frame3, "Fechar ao Criar", fechar_ao_criar, update_config_from_widgets)
